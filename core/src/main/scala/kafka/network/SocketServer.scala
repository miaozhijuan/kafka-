...
class SocketServer(val config: KafkaConfig, val metrics: Metrics, val time: Time) extends Logging with KafkaMetricsGroup {
  ...
  def startup() {
    this.synchronized {
      ...
      //endpoints
      //配置Kafka的时候
      //config/server.properties  listeners = PLAINTEXT://your.host.name:9092
      //  hadoop1:9092
      //  hadoop1:9093
      //  hadoop1:9094
      //不过我们一般是不会这样去设置的。
      ...
      endpoints.values.foreach { endpoint =>
        ...
        for (i <- processorBeginIndex until processorEndIndex)
          //说明这个里面会创建3个线程
         //默认是3个线程，但是我们一般 搭建kafka集群的时候回去配置这个参数。
          processors(i) = newProcessor(i, connectionQuotas, protocol)
        //scala里面有一个函数：主构造函数。
        //我们new一个类，会让这个类的主构造函数去运行。
...
private[kafka] class Processor(val id: Int,
  ...
  private def processNewResponses() {
    //因为我们有多个Process线程，一个线程就对应一个响应队列。
    //这个id就是一个Process线程的编号。

    //TODO 所以curr 其实就是我们说的一个Response对象
